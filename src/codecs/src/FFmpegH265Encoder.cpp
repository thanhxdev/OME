...
FFmpegH265Encoder::FFmpegH265Encoder() : m_impl(new Impl()) {}

FFmpegH265Encoder::~FFmpegH265Encoder() {
    (void)Stop();
    m_impl->Cleanup();
}

std::string FFmpegH265Encoder::GetName() const { return "FFmpegH265Encoder"; }

core::PipelineState FFmpegH265Encoder::GetState() const { return m_impl->state; }

core::VoidResult FFmpegH265Encoder::Initialize() {
    ...
    if (!codec) {
        return std::unexpected(core::Error::Make(core::ErrorCode::CodecNotFound, "HEVC (H265) encoder not found"));
    }
    ...
    if (codec->id == AV_CODEC_ID_HEVC) {
        av_opt_set(m_impl->codecContext->priv_data, "preset", m_impl->config.preset.empty() ? "ultrafast" : m_impl->config.preset.c_str(), 0);
        av_opt_set(m_impl->codecContext->priv_data, "tune", "zerolatency", 0);
...
core::VoidResult FFmpegH265Encoder::Start() {
    std::lock_guard lock(m_impl->mutex);
...
core::VoidResult FFmpegH265Encoder::Stop() {
    std::lock_guard lock(m_impl->mutex);
    m_impl->state = core::PipelineState::Stopped;
    return {};
}

core::VoidResult FFmpegH265Encoder::PushFrame(std::shared_ptr<core::MediaFrame> frame) {
    std::lock_guard lock(m_impl->mutex);
...
core::Result<std::shared_ptr<core::MediaFrame>> FFmpegH265Encoder::PullFrame() {
    std::lock_guard lock(m_impl->mutex);
    if (m_impl->state != core::PipelineState::Running) {
        return std::unexpected(core::Error::Make(core::ErrorCode::InvalidState, "Encoder is not running"));
    }
...
core::VoidResult FFmpegH265Encoder::Connect(std::shared_ptr<core::IMediaObject> downstream) {
    std::lock_guard lock(m_impl->mutex);
    m_impl->downstream = downstream;
    return {};
}

core::VoidResult FFmpegH265Encoder::Disconnect() {
    std::lock_guard lock(m_impl->mutex);
    m_impl->downstream.reset();
    return {};
}

void FFmpegH265Encoder::OnStateChange(core::StateChangeCallback callback) {
    m_impl->onStateChange = callback;
}

void FFmpegH265Encoder::OnError(core::ErrorCallback callback) {
    m_impl->onError = callback;
}

core::VoidResult FFmpegH265Encoder::Configure(const EncoderConfig& config) {
    std::lock_guard lock(m_impl->mutex);
