...
FFmpegOpusEncoder::FFmpegOpusEncoder() : m_impl(new Impl()) {}
...
std::string FFmpegOpusEncoder::GetName() const { return "FFmpegOpusEncoder"; }

core::PipelineState FFmpegOpusEncoder::GetState() const { return m_impl->state; }

core::VoidResult FFmpegOpusEncoder::Initialize() {
    ...
    if (!codec) {
        return std::unexpected(core::Error::Make(core::ErrorCode::CodecNotFound, "Opus encoder not found"));
    }
    ...
    m_impl->codecContext->sample_fmt = AV_SAMPLE_FMT_FLT;
    m_impl->codecContext->time_base = {1, m_impl->config.sampleRate};

    int ret = avcodec_open2(m_impl->codecContext, codec, nullptr);
    if (ret < 0) {
        return std::unexpected(core::Error::Make(core::ErrorCode::CodecOpenFailed, "Could not open Opus codec"));
    }
...
core::VoidResult FFmpegOpusEncoder::Start() {
    std::lock_guard lock(m_impl->mutex);
...
core::VoidResult FFmpegOpusEncoder::Stop() {
    std::lock_guard lock(m_impl->mutex);
    m_impl->state = core::PipelineState::Stopped;
    return {};
}

core::VoidResult FFmpegOpusEncoder::PushFrame(std::shared_ptr<core::MediaFrame> frame) {
    std::lock_guard lock(m_impl->mutex);
...
core::Result<std::shared_ptr<core::MediaFrame>> FFmpegOpusEncoder::PullFrame() {
    std::lock_guard lock(m_impl->mutex);
    if (m_impl->state != core::PipelineState::Running) {
        return std::unexpected(core::Error::Make(core::ErrorCode::InvalidState, "Encoder is not running"));
    }
...
core::VoidResult FFmpegOpusEncoder::Connect(std::shared_ptr<core::IMediaObject> downstream) {
    std::lock_guard lock(m_impl->mutex);
    m_impl->downstream = downstream;
    return {};
}

core::VoidResult FFmpegOpusEncoder::Disconnect() {
    std::lock_guard lock(m_impl->mutex);
    m_impl->downstream.reset();
    return {};
}

void FFmpegOpusEncoder::OnStateChange(core::StateChangeCallback callback) {
    m_impl->onStateChange = callback;
}

void FFmpegOpusEncoder::OnError(core::ErrorCallback callback) {
    m_impl->onError = callback;
}

core::VoidResult FFmpegOpusEncoder::Configure(const EncoderConfig& config) {
    std::lock_guard lock(m_impl->mutex);
