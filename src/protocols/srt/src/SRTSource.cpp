...
bool SRTSource::Connect(const std::string& uri) {
    ...
    if (m_isListener) {
        // Listener mode
        int blocking = 1; // 1 = blocking, 0 = non-blocking (default is 1 for rcv)
        ...
        // Note: srt_accept is blocking here. In a real application, you might want 
        // to do this in a separate thread.
        ...
        spdlog::info("SRT client connected");
    } else {
        // Caller mode
...
bool SRTSource::GetStatistics(SRTStatistics& stats) const {
    ...
    stats.pktLossTotal = bstats.pktRcvLossTotal;
    stats.mbpsBandwidth = bstats.mbpsBandwidth;
    // stats.pktRetransmitTotal = bstats.pktRcvRetransTotal;
