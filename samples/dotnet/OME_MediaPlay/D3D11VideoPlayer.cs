                        ...
                        if (sharedTex != null)
                        {
                            try
                            {
                                ...
                                if (backBuffer != null)
                                {
                                    ...
                                    if (_frameCount % 60 == 0)
                                    {
                                        System.Diagnostics.Debug.WriteLine($"RenderLoop: Rendered {_frameCount} frames. Index: {_currentBufferIndex}");
                                        Console.WriteLine($"RenderLoop: Rendered {_frameCount} frames. Index: {_currentBufferIndex}");
                                    }
                                }
                            }
                            catch (Exception ex)
                            {
                                System.Diagnostics.Debug.WriteLine($"RenderLoop error: {ex.Message}");
                            }
                        }
                    }
                }
