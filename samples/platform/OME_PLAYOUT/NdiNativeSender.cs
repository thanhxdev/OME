...
namespace OME_PLAYOUT
{
    ...
    {
        public static async Task<List<string>> ScanSourcesAsync(uint timeoutMs = 1500)
        {
    ...
    public sealed class NdiNativeSender : IDisposable
    {
        ...
        public void FeedVideo(byte[] bgraBytes, int width, int height, double fps = 59.94)
        {
            SendVideoFrame(bgraBytes, width, height, fps);
        }

        public void FeedAudio(byte[] pcmBytes, int byteCount, int sampleRate = 48000, int channels = 2)
        {
            SendAudioFrame(pcmBytes, byteCount, sampleRate, channels);
        }
