...
namespace WEBRTC_ENCODE
{
    ...
    public sealed class AudioMeterService : IDisposable
    {
        ...
        /// <summary>
        /// Cập nhật dữ liệu đo kiểm cho một kênh đơn lẻ (ví dụ: kênh Micro ở vị trí cuối cùng).
        /// </summary>
        public void TapSingleChannel(ReadOnlySpan<float> monoSamples, int channelIndex, int sampleRate = 48000)
        {
            if (monoSamples.Length == 0 || channelIndex < 0 || channelIndex >= MAX_CHANNELS) return;

            float sr = sampleRate > 0 ? sampleRate : 48000.0f;
            double alphaVu = 1.0 - Math.Exp(-1.0 / (sr * 0.300));

            double peakLinear = 0.0;
            int sampleCount = monoSamples.Length;

            lock (_lock)
            {
                for (int i = 0; i < sampleCount; i++)
                {
                    double sample = Math.Abs(monoSamples[i]);
                    if (sample > peakLinear)
                    {
                        peakLinear = sample;
                    }
                    _ewmaEnergy[channelIndex] += alphaVu * (sample * sample - _ewmaEnergy[channelIndex]);
                }

                double peakDb = (peakLinear > NOISE_FLOOR_LINEAR) ? 20.0 * Math.Log10(peakLinear) : MIN_DBFS;
                _currentPeakDb[channelIndex] = Math.Clamp(peakDb, MIN_DBFS, MAX_DBFS);

                double rmsLinear = Math.Sqrt(Math.Max(0.0, _ewmaEnergy[channelIndex]));
                double rmsDb = (rmsLinear > NOISE_FLOOR_LINEAR) ? 20.0 * Math.Log10(rmsLinear) : MIN_DBFS;
                _currentRmsDb[channelIndex] = Math.Clamp(rmsDb, MIN_DBFS, MAX_DBFS);

                if (_currentPeakDb[channelIndex] > _peakHoldDb[channelIndex])
                {
                    _peakHoldDb[channelIndex] = _currentPeakDb[channelIndex];
                }
                else
                {
                    _peakHoldDb[channelIndex] = Math.Max(MIN_DBFS, _peakHoldDb[channelIndex] - 0.2);
                }

                _isClipping[channelIndex] = (peakLinear >= 0.988);
            }
        }
