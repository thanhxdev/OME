...
namespace WEBRTC_ENCODE
{
    ...
    {
        public static Task<List<string>> FindSourcesAsync(int timeoutMs = 1500)
        {
            return Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    int elapsed = 0;
                    int step = 300;
                    int totalTimeout = Math.Max(1000, timeoutMs);

                    while (elapsed < totalTimeout)
                    {
                        NdiNativeApi.NDIlib_find_wait_for_sources(finder, (uint)step);
                        elapsed += step;
                        ...
                        if (result.Count > 0 && elapsed >= 600)
                        {
                            // Found sources and gave sufficient time for announcements
                            break;
                        }
                    }
                }
                    ...
                    switch (frameType)
                    {
                        ...
                        case NDIlib_frame_type_e.Audio:
                            try
                            {
                                ...
                                if (audioFrame.p_data != IntPtr.Zero && samples > 0 && channels > 0)
                                {
                                    // NDI delivers 32-bit float audio in planar format (non-interleaved).
                                    // De-planarize / interleave into standard [L0, R0, L1, R1, ...] format
                                    float[] interleavedBuffer = new float[samples * channels];
                                    int strideBytes = audioFrame.channel_stride_in_bytes > 0
                                        ? audioFrame.channel_stride_in_bytes
                                        : samples * sizeof(float);

                                    unsafe
                                    {
                                        byte* basePtr = (byte*)audioFrame.p_data.ToPointer();
                                        for (int ch = 0; ch < channels; ch++)
                                        {
                                            float* chPtr = (float*)(basePtr + (ch * strideBytes));
                                            for (int s = 0; s < samples; s++)
                                            {
                                                interleavedBuffer[s * channels + ch] = chPtr[s];
                                            }
                                        }
                                    }

                                    AudioSamplesReceived?.Invoke(interleavedBuffer, channels, sampleRate);
                                }
                            }
                            finally
                            {
