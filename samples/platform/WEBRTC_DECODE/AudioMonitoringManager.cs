...
namespace WEBRTC_DECODE
{
    ...
    public sealed class AudioMonitoringManager : IDisposable
    {
        ...
        // Studio Mic / Director Talkback state
        private bool _enableStudioMic = false;
        public bool EnableStudioMic
        {
            get => _enableStudioMic;
            set
            {
                _enableStudioMic = value;
                if (!value)
                {
                    _studioMicRingBuffer.Clear();
                    _studioMicSpeakerRingBuffer.Clear();
                    _studioMicPgmRingBuffer.Clear();
                }
            }
        }
        ...
        {
            get => _studioMicRouteToProgram;
            set
            {
                _studioMicRouteToProgram = value;
                if (!value)
                {
                    _studioMicPgmRingBuffer.Clear();
                }
        ...
        public int CurrentOutputDeviceId => _audioOutput.CurrentDeviceId;

        // Scratch buffers for mixer loop to prevent GC allocations
        private readonly byte[] _tempChannelPcm = new byte[9600];
        private readonly byte[] _tempMicPcm = new byte[9600];
        private byte[] _programPcm = new byte[9600];
        ...
        public event Action<ChannelAudioLevels>? StudioMicLevelsUpdated;
        ...
        public event Action<int, byte[], int>? MixMinusPcmAvailable;

        public bool EnableMixMinus { get; set; } = true;
        ...
        public AudioChannelConfiguration ChannelConfig
        {
            ...
            set
            {
                ...
                _studioMicMeter.ActiveChannelCount = 2; // Studio Mic is Stereo
        ...
        public bool IsPreviewMuted
        {
            ...
            set
            {
                ...
                Log("[AUDIO]", _isPreviewMuted 
                    ? "🔇 MUTE âm thanh Preview loa kiểm âm (âm thanh luồng Stream/NDI vẫn phát 100%)" 
                    : "🔊 UNMUTE âm thanh Preview loa kiểm âm");
                _audioOutput.Wake();
            }
        }
        ...
        /// <summary>
        /// Feed incoming Intercom audio from camera encoders into the director's monitoring mixer.
        /// </summary>
        public void FeedIntercomIncomingPcm(byte[] pcmData, int count)
        {
            if (pcmData == null || count <= 0) return;
            _intercomRingBuffer.Write(pcmData, 0, count);
            _audioOutput.Wake();
        }
        ...
        private int ReadMixedPcm(byte[] destination, int offset, int count)
        {
            ...
            // Blend Director Mic into Program Master if ChkStudioMicToProgram is checked (Ghép audio Micro Đạo Diễn vào PGM Master)
            if (EnableStudioMic && StudioMicRouteToProgram && _studioMicPgmRingBuffer.AvailableBytes >= 4)
            {
                if (_studioMicPgmRingBuffer.AvailableBytes > 19200)
                {
                    _studioMicPgmRingBuffer.Clear();
                }
                int dirPgmBytes = _studioMicPgmRingBuffer.Read(_tempMicPcm, 0, Math.Min(count, _tempMicPcm.Length));
                if (dirPgmBytes > 0)
                {
                    float micGain = (StudioMicGainDb <= -58.0) ? 0.0f : (float)Math.Pow(10.0, StudioMicGainDb / 20.0);
                    int micFrames = dirPgmBytes / 4;
                    for (int n = 0; n < Math.Min(frameCount, micFrames); n++)
                    {
                        int byteIdx = n * 4;
                        short micL = BitConverter.ToInt16(_tempMicPcm, byteIdx);
                        short micR = BitConverter.ToInt16(_tempMicPcm, byteIdx + 2);
                        _mixL[n] += (micL / 32768.0f) * micGain;
                        _mixR[n] += (micR / 32768.0f) * micGain;
                    }
                }
            }

            // 1. Program Master Stream Stage: Always emit clean Program Audio with Limiter to NDI, SDI, Recording & SRT Bridge.
            // Strictly isolated from Intercom incoming/talkback so it NEVER leaks into broadcast air.
            if (_programPcm.Length < offset + count)
            {
                _programPcm = new byte[offset + count];
            }

            for (int n = 0; n < frameCount; n++)
            {
                float pgmSampleL = SoftLimit(_mixL[n]);
                float pgmSampleR = SoftLimit(_mixR[n]);

                short pgmOutL = (short)Math.Clamp(pgmSampleL * 32767.0f, short.MinValue, short.MaxValue);
                short pgmOutR = (short)Math.Clamp(pgmSampleR * 32767.0f, short.MinValue, short.MaxValue);

                int pgmIdx = offset + (n * 4);
                _programPcm[pgmIdx] = (byte)(pgmOutL & 0xFF);
                _programPcm[pgmIdx + 1] = (byte)((pgmOutL >> 8) & 0xFF);
                _programPcm[pgmIdx + 2] = (byte)(pgmOutR & 0xFF);
                _programPcm[pgmIdx + 3] = (byte)((pgmOutR >> 8) & 0xFF);
            }

            // Feed actual mixed Program PCM into program meter (VU meters always show stream audio levels)
            _programMeter.ProcessPcmBytes(_programPcm, offset, count, 16, 2, 48000, isFloat: false);

            // Emit mixed Program PCM audio for NDI, SDI, Recording & SRT Bridge outputs (Always active for stream)
            ProgramMixedPcmAvailable?.Invoke(_programPcm, offset, count);

            // 2. Local Speaker Preview Output Stage (to computer soundcard / headphones)
            if (_isPreviewMuted)
            {
                // Preview Mute: zero out the hardware buffer so local speakers make NO Program sound
                Array.Clear(destination, offset, count);
            }
            else
            {
                // Apply master monitor volume fader & mute all
                ...
                for (int n = 0; n < frameCount; n++)
                {
                    ...
                    float sampleL = SoftLimit(_mixL[n] * _currentMasterRamp);
                    float sampleR = SoftLimit(_mixR[n] * _currentMasterRamp);
            ...
            // 3. Intercom Incoming Audio Stage (from Camera Encoders):
            // Strictly routed ONLY to local speaker/headphone destination, NEVER into Program Master stream
            if (_intercomRingBuffer.AvailableBytes >= 4)
            {
                int intercomBytesRead = _intercomRingBuffer.Read(_tempChannelPcm, 0, Math.Min(count, _tempChannelPcm.Length));
                if (intercomBytesRead > 0)
                {
                    float intercomVol = (float)Math.Clamp(IntercomReceiveVolume, 0.0, 2.0);
                    int intercomFrames = intercomBytesRead / 4;
                    for (int n = 0; n < Math.Min(frameCount, intercomFrames); n++)
                    {
                        int dIdx = offset + (n * 4);
                        short curL = BitConverter.ToInt16(destination, dIdx);
                        short curR = BitConverter.ToInt16(destination, dIdx + 2);

                        short icL = BitConverter.ToInt16(_tempChannelPcm, n * 4);
                        short icR = BitConverter.ToInt16(_tempChannelPcm, n * 4 + 2);

                        float sumL = (curL / 32768.0f) + (icL / 32768.0f) * intercomVol;
                        float sumR = (curR / 32768.0f) + (icR / 32768.0f) * intercomVol;

                        short resL = (short)Math.Clamp(sumL * 32767.0f, short.MinValue, short.MaxValue);
                        short resR = (short)Math.Clamp(sumR * 32767.0f, short.MinValue, short.MaxValue);

                        destination[dIdx] = (byte)(resL & 0xFF);
                        destination[dIdx + 1] = (byte)((resL >> 8) & 0xFF);
                        destination[dIdx + 2] = (byte)(resR & 0xFF);
            ...
            if (EnableStudioMic && _studioMicSpeakerRingBuffer.AvailableBytes >= 4)
            {
                ...
                int micBytesRead = _studioMicSpeakerRingBuffer.Read(_tempMicPcm, 0, Math.Min(count, _tempMicPcm.Length));
                // Nếu Director Mic chưa được mix vào Master (hoặc Preview đang bị MUTE không nghe thấy Master),
                // thì cộng Director Mic vào ngõ ra loa kiểm âm destination
                if (micBytesRead > 0 && (!StudioMicRouteToProgram || _isPreviewMuted))
                {
                    float micGain = (StudioMicGainDb <= -58.0) ? 0.0f : (float)Math.Pow(10.0, StudioMicGainDb / 20.0);
                    int micFrames = micBytesRead / 4;
                    for (int n = 0; n < Math.Min(frameCount, micFrames); n++)
                    {
                        int dIdx = offset + (n * 4);
                        short curL = BitConverter.ToInt16(destination, dIdx);
                        ...
                        short inMicL = BitConverter.ToInt16(_tempMicPcm, n * 4);
                        short inMicR = BitConverter.ToInt16(_tempMicPcm, n * 4 + 2);

                        float sumL = (curL / 32768.0f) + (inMicL / 32768.0f) * micGain;
                        float sumR = (curR / 32768.0f) + (inMicR / 32768.0f) * micGain;

                        short resL = (short)Math.Clamp(SoftLimit(sumL) * 32767.0f, short.MinValue, short.MaxValue);
                        short resR = (short)Math.Clamp(SoftLimit(sumR) * 32767.0f, short.MinValue, short.MaxValue);
        ...
        /// <summary>
        /// WebRTC 2.0 Mix-Minus (N-1 Clean Feed) Audio Engine.
        /// Mixes all active camera and studio tracks but strictly EXCLUDES the audio from targetCameraIndex,
        /// ensuring return IFB/Talkback audio sent back to the camera does NOT contain that camera's own voice.
        /// </summary>
        public byte[] GenerateMixMinusPcm(int targetCameraIndex, int frameCount = 960)
        {
            byte[] outBytes = new byte[frameCount * 4]; // 16-bit stereo 48kHz
            float[] mixL = new float[frameCount];
            float[] mixR = new float[frameCount];
            byte[] tempChannelBuffer = new byte[frameCount * 4];

            for (int i = 0; i < MaxCameras; i++)
            {
                // In Mix-Minus mode, strictly exclude the caller's own voice
                if (EnableMixMinus && i == targetCameraIndex) continue;
                if (_channelMuted[i]) continue;

                int bytesRead = _camRingBuffers[i].Peek(tempChannelBuffer, 0, tempChannelBuffer.Length);
                if (bytesRead <= 0) continue;

                float gain = (_channelGainDb[i] <= -58.0) ? 0.0f : (float)Math.Pow(10.0, _channelGainDb[i] / 20.0);
                float pan = (float)Math.Clamp(_channelPan[i], -1.0, 1.0);
                float panAngle = (pan + 1.0f) * 0.25f * (float)Math.PI;
                float panL = (float)Math.Cos(panAngle) * 1.4142f;
                float panR = (float)Math.Sin(panAngle) * 1.4142f;
                ...
                {
                    int byteIdx = n * 4;
                    if (byteIdx + 3 >= bytesRead) break;

                    short rawL = BitConverter.ToInt16(tempChannelBuffer, byteIdx);
                    short rawR = BitConverter.ToInt16(tempChannelBuffer, byteIdx + 2);

                    mixL[n] += (rawL / 32768.0f) * gain * panL;
                    mixR[n] += (rawR / 32768.0f) * gain * panR;
                }
            }
            ...
            if (EnableStudioMic && IsTalkbackActive)
            {
                ...
                if (micBytes > 0)
                {
                    ...
                    {
                        int byteIdx = n * 4;
                        if (byteIdx + 3 >= micBytes) break;

                        short micL = BitConverter.ToInt16(tempChannelBuffer, byteIdx);
                        short micR = BitConverter.ToInt16(tempChannelBuffer, byteIdx + 2);

                        mixL[n] += (micL / 32768.0f) * micGain;
            ...
            for (int n = 0; n < frameCount; n++)
            {
                float sL = SoftLimit(mixL[n]);
                float sR = SoftLimit(mixR[n]);
                short outL = (short)Math.Clamp(sL * 32767.0f, short.MinValue, short.MaxValue);
                short outR = (short)Math.Clamp(sR * 32767.0f, short.MinValue, short.MaxValue);

                int dIdx = n * 4;
                outBytes[dIdx] = (byte)(outL & 0xFF);
                outBytes[dIdx + 1] = (byte)((outL >> 8) & 0xFF);
                outBytes[dIdx + 2] = (byte)(outR & 0xFF);
                outBytes[dIdx + 3] = (byte)((outR >> 8) & 0xFF);
            }

            MixMinusPcmAvailable?.Invoke(targetCameraIndex, outBytes, outBytes.Length);
            return outBytes;
        }
        ...
        public void ProcessAudioTick(bool[] channelActive, int programIndex)
        {
            ...
            // Update Studio Mic Levels
            if (EnableStudioMic)
            {
                _studioMicMeter.GetPreviewAudioLevels(tempPeaks, tempRms, tempClips, isAudioActive: true);
                StudioMicLevels.CopyFrom(tempPeaks, tempRms, tempClips, 2);
            }
            else
            {
                _studioMicMeter.DecayMeters();
                _studioMicMeter.GetPreviewAudioLevels(tempPeaks, tempRms, tempClips, isAudioActive: false);
                StudioMicLevels.CopyFrom(tempPeaks, tempRms, tempClips, 2);
            }

            CamLevelsUpdated?.Invoke(_camLevels);
            ProgramLevelsUpdated?.Invoke(_programLevels);
            StudioMicLevelsUpdated?.Invoke(StudioMicLevels);
        }
